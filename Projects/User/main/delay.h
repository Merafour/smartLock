...
//extern void set_timeout(const uint32_t ms);
//extern void clrar_timeout(void);
//extern volatile uint32_t get_timeout(void);
extern uint32_t waitting_time(const uint32_t ms);
extern void set_delay_tick(uint16_t delay);
extern uint32_t GetSeconds(void);
extern void SetSeconds(uint32_t sec);

extern uint16_t backlight_delay;
